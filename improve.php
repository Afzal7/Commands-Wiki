...
*/	include 'sql.php';
	...
	{
		$query='create table if not exists improvements(
		...
		else if($type=='option')
		{
   		...
   		{
	   		$check2="select * from improvements where command_id='".$id."' and command=(".$ccc.") and text ='".$text."' ";
	   		$c2=mysqli_query($con,$check2) or die('Error in checking data!!');
	   		
	   		if(mysqli_num_rows($c2)>0)
	   		{
	   			echo 'The same data has already been submitted before...';
	   		}
	   		else
	   		{
		   		$insert="insert into improvements(command_id,command,name,text) VALUES('".$id."',(".$ccc."),'".$name."','".$text."')";
		   		$i=mysqli_query($con,$insert) or die('Error in Improvement !!!');
		   		echo 'Improvement Submitted :)'; 
			}
		}
   		mysqli_close($con);
	}
